...
describe("项目换日规则文档", () => {
  it.each(文档内容)("%s 已同步为23点进子时、午夜换日", (_名称, 内容) => {
    expect(内容).toContain("23:00 进入子时");
    expect(内容).toContain("00:00 换日");
    expect(内容).not.toContain("23:00 起进入下一传统历法日");
  });

  it("开发规则明确区分公历今天与传统历法计算日", () => {
    const 开发规则 = 文档内容.find(([名称]) => 名称 === "开发规则.md")?.[1] ?? "";
    expect(开发规则).toContain("公历月历日期和“今天”标记仍按普通北京时间公历日期显示");
    expect(开发规则).toContain("真太阳时模式按真太阳时自身钟点判断");
    expect(开发规则).toContain("均为 23:00 进入子时、00:00 换日");
  });
});
