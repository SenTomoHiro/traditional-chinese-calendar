...
describe("神圣纪念日自然语言匹配", () => {
  ...
  it("正月十五保留同日、范围和每月重复的全部事件", () => {
    const 事件 = 获取神圣纪念日(配置, 农历(1, 15, "正月", "十五"));
    expect(事件).toEqual(
      expect.arrayContaining(["上元天官圣诞", "门神户尉圣诞", "佑圣真君圣诞", "显大神通降魔", "北斗下降之辰"]),
    );
    expect(事件.length).toBeGreaterThanOrEqual(8);
  });
  ...
  it("每月初八在不同正常月份重复匹配", () => {
    expect(获取神圣纪念日(配置, 农历(2, 8, "二月", "初八"))).toContain("北斗下降之辰");
    expect(获取神圣纪念日(配置, 农历(11, 8, "十一月", "初八"))).toContain("北斗下降之辰");
  });
