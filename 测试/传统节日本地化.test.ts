...
describe("1900—2100传统节日基线与本地民俗扩展", () => {
  it("逐日保持1.8.6基线，仅加入两项已定传统民俗宗教节日", () => {
    ...
    for (let 毫秒 = 开始; 毫秒 <= 结束; 毫秒 += 86_400_000) {
      ...
      if (上游农历.getMonth() === 4 && 上游农历.getDay() === 15) 上游.push("佛吉祥日（卫塞节）");
      if (上游农历.getMonth() === 10 && 上游农历.getDay() === 8) 上游.push("民俗涅槃日（放生日）");
